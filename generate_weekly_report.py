#!/usr/bin/env python3
"""
Weekly Trustpilot Report Generator
- Uses ONE universal template for all brands
- Copies template and fills with brand-specific data
- Clean, maintainable approach with placeholders
"""
...
# =============================================================================
# UNIVERSAL TEMPLATE CREATION (One-time setup)
# =============================================================================
...
"""
    ...
    print(f"\nYou can customize this template (fonts, colors, layout) and it will apply to all brands!")
...
def prepare_report_data(brand_id, week_start, week_end):
    """Prepare all data from database"""
    ...
    previous_week = snapshots[1] if len(snapshots) > 1 else None
    
    # Get THIS WEEK's reviews only
    reviews = database.get_reviews_for_week(brand_id, week_start, week_end)
    
    # Language breakdown - current week only
    language_counts = {}
    for review in reviews:
        lang = review.get('language', 'unknown').upper()
        language_counts[lang] = language_counts.get(lang, 0) + 1
    
    # Format languages as bullet list
    ...
    # Topics
    topics = database.get_topics_by_sentiment(brand_id, week_start, week_end)
    ...
    return {
        ...
        'date_range': f"{datetime.fromisoformat(week_start).strftime('%B %d, %Y')} → {datetime.fromisoformat(week_end).strftime('%B %d, %Y')}",
...
def fill_template(services, doc_id, data, chart_urls):
    """Fill template with actual data using replaceAllText"""
    ...
    for key, value in data.items():
        if key not in ['snapshots', 'current_week']:  # Skip non-text data
            requests.append({
                'replaceAllText': {
                    'containsText': {
                        'text': f'{{{{{key}}}}}',
    ...
    # Now replace chart placeholders with images
    chart_replacements = [
    ...
    for placeholder, url in chart_replacements:
        if url:
            # Get document to find placeholder
...
def generate_report(brand_id):
    ...
    # Get or create universal template
    ...
    # Prepare data
    data = prepare_report_data(brand_id, week_start.isoformat(), week_end.isoformat())
    if not data:
